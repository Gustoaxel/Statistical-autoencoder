...
def basic_run_eta_compare(func_algo, func_predict,
    ...
    for i,(train_ind, test_ind) in enumerate(kf.split(YR)):
        ...
        accuracy_train[i,0],accuracy_train[i,1:k+1] = compute_accuracy(Ytrain,Ytrain_pred,k)
        accuracy_test[i,0],accuracy_test[i,1:k+1] = compute_accuracy(Ytest,Ytest_pred,k)
        if np.unique(Ytest).shape[0] == 2  and np.unique(Ytest_pred.astype('int64')).shape[0] == 2: 
        ...
        for j in range(numalg):
            ...
            if k>2 : 
                Ypred_test = np.around(model.predict(Xtest)).ravel() #getPredLabel(model.predict(Xtest))
                Ypred_train = np.around(model.predict(Xtrain)).ravel() #getPredLabel(model.predict(Xtrain))
            else : 
            ...
            if np.unique(Ytest).shape[0] == 2  and np.unique(Ypred_test.astype('int64')).shape[0] == 2: 
                AUC_test_comp[i][j*4] = roc_auc_score(Ypred_test.astype('int64'),Ytest)
                AUC_train_comp[i][j*4] = roc_auc_score(Ypred_train.astype('int64'),Ytrain)
            
            # F1 precision recal 
            AUC_train_comp[i][j*4 +1 :j*4+4] = metrics.precision_recall_fscore_support(Ytrain, Ypred_train.astype('int64'), average='macro')[:-1]
            AUC_test_comp[i][j*4 +1:j*4+4] = metrics.precision_recall_fscore_support(Ytest,Ypred_test.astype('int64'), average='macro')[:-1]
    ...
    return mu_mean, nbm, accG, loss, W_mean, timeElapsed, \
            df_topGenes, df_normW, df_topG_normW, \
            df_topGenes_mean, df_normW_mean, df_topG_normW_mean,\
            df_acctest,df_comp,df_timeElapsed,w_all, df_compauc
import warnings

def readData(filename) : 
        DATADIR = 'Test_datasets/'
    #    df_X = pd.read_csv(DATADIR+'LUNG.csv',delimiter=';', decimal=",",header=0,encoding="ISO-8859-1", low_memory=False)
        df_X = pd.read_csv(DATADIR+str(filename),delimiter=';', decimal=",",header=0,encoding="ISO-8859-1", low_memory=False)
...
def basic_run_other(
    ...
    return df_comp,df_timeElapsed, df_compauc
